<template>
  ...
  >
    <div class="flex justify-between h-22 w-full pr-6 pb-2 bg-gray-100">
      ...
      <button
        @click="save"
        class="inline-block h-10 rounded-md border border-transparent px-4 bg-blue-600 text-base my-auto leading-6 font-medium text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5"
      >
        Save
      </button>
    </div>
    <div class="h-16 bg-gray-100 w-full">
      <div class="pl-6 flex items-baseline ">
        <div
          @click="pane = 0"
          :class="
            pane == 0 ? `${style.tabs.enabled}` : `${style.tabs.disabled}`
          "
        >
          Core
        </div>
        <div
          ...
          :class="
            pane == 1
              ? `${style.tabs.enabled} ml-3`
              : `${style.tabs.disabled} ml-3`
          "
        >
          Console
        </div>
    ...
    <div v-if="pane == 0" class="flex flex-col">
      <div class="relative w-full overflow-hidden">
        <div class="w-full overflow-hidden h-full">
          <div
            v-for="(setting, s) in Object.keys(pending)"
            :key="`setting-${s}`"
          >
            <div
              class="w-full px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider"
            >
              {{ setting }} Options
            </div>
                ...
                <div class="w-auto px-6 pt-6">
                  <div class="flex">
                    <div class="w-full relative">
                      ...
                      <div
                        class="text-sm leading-5 text-gray-500 w-auto truncate"
                      >
                        {{ param.desc }}
                      </div>
                    </div>
                ...
                <div class="px-6 pt-4 mb-6">
                  <div class="">
                    <div v-if="typeof param.value == 'string'">
                      <div v-if="p == 'password'">
                        <div
                          class="h-10 opacity-50 block w-full p-2 sm:text-sm sm:leading-5"
                          v-if="pending.security.doNotRemember.value"
                        >
                          N/A
                        </div>
                        <div v-else>
                          <input
                            ...
                            v-model="param.value"
                            class="form-input block w-full p-2 sm:text-sm sm:leading-5 h-10"
                            :placeholder="p"
                          />
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        :class="
                          param.value
                            ...
                            : 'inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-gray-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-gray-500 focus:outline-none focus:border-gray-700 focus:shadow-outline-gray transition ease-in-out duration-150 sm:text-sm sm:leading-5'
                        "
                      >
        ...
        >
          <table class="min-w-full">
            ...
            <tbody class="bg-gray-200 text-sm">
              <tr
                v-for="(c, i) in $store.getters.getConsole"
                :key="'console-' + i"
              >
                <td
                  ...
                  <div class="flex items-center">
                    <div>
                      <div class="font-mono text-gray-400">
                        {{ Date(c.timestamp) | timeAgo }}
                      </div>
                    </div>
                    <div class="ml-4">
                      <div class="leading-5 font-mono text-gray-900">
                        {{ c.line }}
                      </div>
                    </div>
                  </div>
  ...
  data() {
    return {
      ...
      style: {
        tabs: {
          enabled:
            "px-3 py-2 rounded-md text-sm font-medium text-gray-100 bg-gray-700 transition ease-in-out duration-150",
          disabled:
            "cursor-pointer px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:text-white hover:bg-gray-300 focus:outline-none focus:text-white focus:bg-gray-700 transition ease-in-out duration-150"
        }
      }
    };
  },
